...
var currentHitbox = {NORMAL = Vector2(9,19)*2, ROLL = Vector2(7,14)*2, CROUCH = Vector2(9,11)*2, GLIDE = Vector2(10,10)*2, HORIZONTAL = Vector2(22,9)*2}
...
# use Global.CHARACTERS to refer to this variable's value
...
# animations in the array are played in order, left to right
# note: idle is always played first (standing animation)
var playerIdles: Array[String] = ["idle1"]
var characterName = "Sonic" # This is for level intermissions referring to the player character by name
...
func _ready():
	...
	# run switch physics to ensure knuckles gets his jump height nerfed
	switch_physics()
	
	# set character specific variables
	spriteController.set_variables()
